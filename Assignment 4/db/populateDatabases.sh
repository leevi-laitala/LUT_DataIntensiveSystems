...
psql -U "$PG_DB_USER" --host "$PG_DB_HOST" --port "$PG_DB_PORT" -d "postgres" -c "DROP DATABASE IF EXISTS $PG_DB_NAME;"
psql -U "$PG_DB_USER" --host "$PG_DB_HOST" --port "$PG_DB_PORT" -d "postgres" -c "CREATE DATABASE $PG_DB_NAME;"
